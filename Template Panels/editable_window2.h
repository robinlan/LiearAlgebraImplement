...
#define IDC_SETTING_BUTTON_2  101
#define IDC_SETTING_EDITBOX_2 102

LRESULT CALLBACK EditableWindowProcedure2(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) {

	static HWND prevHwnd;
	static HWND hSettingEdit2;
	static string resultString2;

	switch(Message) {

		case WM_CREATE: {
			// Create an edit box
			WinButton button1("OK",10,10,80,24,hwnd,(HMENU)IDC_SETTING_BUTTON_2);
			HWND hbutton = button1.getButton();
			WinEditbox editbox1("",110,10,80,24,hwnd,(HMENU)IDC_SETTING_EDITBOX_2);
			hSettingEdit2 = editbox1.getEditbox();
			break;
		}
		...
		case WM_COMMAND: {
			...
			switch(LOWORD(wParam)) {
				...
				case IDC_SETTING_BUTTON_2:
				{
					char buffer[256];
					SendMessage(hSettingEdit2,
						WM_GETTEXT,
